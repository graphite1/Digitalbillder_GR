...
- 管理メニュー → **保管済み履歴・工事別実績**: Webの保管済み査定額を工事・会社・工種で集計します。初回は詳細取得が必要です。「追加・変更分を確認」は保管済み一覧CSVを毎回取得して照合し、確認済み明細を再利用します。「全件を再検証」は過去の査定変更も読み直します。
- 請求詳細 → **履歴候補**: 会社別の使用頻度・金額を参考に工種を選びます。振分金額の自動確定はしません。未登録工種は選択してこの工事へ追加できます。
- 管理メニュー → **工事予算・着地見込**: PDFの2ページ目をセル構造から読み、コード・科目・実行予算・予定金額を候補にします。原本の3桁コードは保持し、照合用にはWeb実績の正式コード（例: `513` → `D513`）を提案します。既存行は **Web工種を照合** で未設定分を補えます。登録前・登録後の手動編集に対応し、元PDFと抽出値を保持します。[使い方と読取限界](docs/PROJECT_BUDGET.md)。

...
| 開発本体に追加・配布前 | 一覧の配置・請求月配色、処理状況表示と他画面の並行操作、取得処理の中断（保存開始前まで）、管理者専用の請求月自動判定リセット、D付き工種テンプレートと3桁入力、確認付きの税額±1円調整 |
...
送信・承認をアプリが自動実行する構想ではありません。画像は公開版についての上記日付のスナップショットです。同日追加した配布前の改善は上表に記載しています。優先順位全体と最新情報は [STATUS_AND_ISSUES.md](STATUS_AND_ISSUES.md) を参照してください。

...
- 確定ソース `ed216a39ae6511490bd4a55a96117c02a7c7f82a`、code sequence 10、Windows v2.0.1-r1 / installer sequence 10 / build 1。既存鍵 `release-2026-01` で署名し、公開済みの旧ファイルを差し替えず新しい配布として公開した。
- 全406テスト成功。請求・添付・取込状態を同一トランザクションで確定し、添付登録に失敗した請求を完了扱いにしないこと、PDFを一時ファイルから原子的に保存すること、候補状態を請求登録と一緒に確定することを確認した。実Web・実請求・主台帳は変更していない。
- ローカル更新・起動試験: `作業補助/配布検証/v2.0.1/local-20260908T140635Z-726eea9b/result.json`。Windows portable runtime試験: `作業補助/配布検証/v2.0.1/packaged-20260908T140645Z/result.json`。いずれも隔離台帳と合成データを使い、署名・適用・起動を確認した。
- 公開取得によるv1.0.8→v2.0.1更新・起動と、コードZIP・Windows ZIP・セットアップEXEの一致確認: `作業補助/配布検証/v2.0.1/public-20260908T141927Z-4a99bdc0/result.json`。コードZIPは814,159 bytes、SHA256 `bccb96a58e14a9f17697e6fa1d4773377c371d0ed525e39f0ba0635dd8cdf272`。Windows ZIPは88,893,597 bytes、SHA256 `41f0da9568914188943c43bae5bb8a355a35fd8768c3bdd7ff1e186cfef50667`。セットアップEXEは99,840 bytes、SHA256 `d1b66f4f1f1dd8cd2bfc9c310de54a7b20032ef15ce5aaef49d127834197cf81`。
- 配布サイトソース `8cdd8f7` / Sites版13を公開。公開サイトからダウンロードした全配布物がローカル署名済み候補に一致した。対象Latitudeでの81件実PDF取込、空のWindows環境での新規セットアップ、実機の速度・メモリは未検証であり、利用試験で確認する。実データ・資格情報・署名秘密鍵は公開物に含めていない。

## 2026-09-07: v1完成版 v1.0.9公開

- 確定ソース `3af318d6ffb7c1711fcd962c1c3507cbf09aa180`、code sequence 9、Windows v1.0.9-r1 / installer sequence 9。既存鍵 `release-2026-01` で署名。公開済みファイルを置換せず、停止報告のあるv1.0.8のコード・Windows配布を停止した。新規取得はv1.0.9を使用する。
- 全397テスト成功、スキップ0。81件全選択、部分選択、描画遅延、選択解除、タイムアウト、中断、対象外PDF拒否、同時・後日の別ID重複保留、予算工種照合・手動設定保持、原本差替え・コピー競合・破損拒否、画面切替・閉じる・遅延結果・再試行を確認。
- 実Webでは81件を50件→31件として選択CSVのID・内容を照合。本体の選択処理を使用し、PDF部分は合成ZIPに置換した。実PDF0件、主台帳ハッシュ不変。結果 `作業補助/読取り試験/all-selection-controls-20260907T095433Z/result.json`。利用PCの81件停止は根因未確定で、81件実PDF取込の完了を確認したものではない。
- 公開取得によるv1.0.8→v1.0.9: `作業補助/配布検証/v1.0.9/public-20260907T101859Z-f205620f/result.json`。v1.0.3→v1.0.9直接更新: `public-20260907T101906Z-11839332/result.json`（同じ親フォルダー）。署名・互換性・適用・起動・合成請求・手動設定・更新前バックアップ・後片付けを確認。旧v1.0.3は税額調整列を初期値0で追加し、既存値を保持する。
- Windows専用Python3.14.3、同じruntime fingerprint、Edge headless、起動EXEの初期化に成功。同梱環境で合成PDFを専用子プロセス解析し、中断・再試行・原本ハッシュ保持・不要DB作成なしを確認。結果 `作業補助/配布検証/v1.0.9/packaged-20260907T101414Z/result.json`。試験用展開runtimeは終了後に削除。
- 公開コードZIP: 809,138 bytes、SHA256 `4a1476191f353e3da4c3aacd431814334163a04051275395b5c0b4b58cab5c49`。Windows ZIP: 88,888,420 bytes、SHA256 `14b8e8c7a5f993a1a535c6704028dbad20e43d3a18987648dbc8cea817226362`。セットアップEXE: 99,840 bytes、SHA256 `6b3d63bb0f73d456c5dc84991468bf41027499a6b2b8212fbf57c30c575c2f4d`。公開取得した全ファイルと署名manifestがローカル候補に一致。結果 `作業補助/配布検証/v1.0.9/public-artifacts.json`。
- 配布サイトソース `06249005f1dff11ccf11cafc49fb2c353bbe5328` / Sites版12を公開。公開HTMLの1.0.9と新EXE案内、latestの署名情報、旧8のコード・Windows取得が404となることを実HTTP確認。一時公開権限を期限切れにし、環境revision 23の反映成功と同資格情報での管理API 403を確認した。利用PCへの適用、対象Latitudeでの速度・メモリ実測、空のWindows環境での新規セットアップ実行は未検証。実データや資格情報は公開物に含めていない。

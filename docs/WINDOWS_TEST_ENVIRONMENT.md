...
最新状況（2026-09-06 11:33）: Windows Update画面からの修復更新再起動後、Sandbox・Hyper-Vの有効化が両方成功した。以前の0x80073712は今回の有効化では出ていない。**機能反映のため、あと1回の通常再起動が必要。ゲスト起動・導入試験はまだ未実施。** 新しい試験一式は `作業補助/WindowsTest/runs/20260906-113324-ee86dc3a/`。再起動後に同フォルダーのWSBで初回導入を試す。更新監視は停止済み。[最新状況と証跡](WINDOWS_REPAIR_DIAGNOSIS.md)を参照。
...
- DISM標準修復は2026-09-06 08:29に終了コード `0x800f0915`（修復コンテンツが見つからない）で失敗した。100%表示は成功を意味しなかった。機能有効化の再試行には進んでいない。現在のWindowsに適合する修復ソースを用意し、修復後に機能を有効化する必要がある。再起動だけで解消したとは扱わない。
